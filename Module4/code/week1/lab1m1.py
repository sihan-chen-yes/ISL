...
msg = "Now you're just some value that I used to nonce"
obj = {"command": "get_signature", "msg":msg}
json_send(obj)
resp_dict = json_recv()
r = resp_dict["r"]
...
k = ecdsa2.Z_q(h)
xr = s * k - h * h
h2 = bits_to_int(hash_message_to_bits("gimme the flag"), q)
s = (1 / k) * (h2 * h2 + xr)
obj = {"command": "solve", "r":r, "s":int(s)}
json_send(obj)
print(json_recv())